...
use std::{net::{IpAddr, TcpStream}, process::Command, usize, io::Write};
...
impl VbanSender {
    ...
    pub fn create(peer : (IpAddr, u16), local_addr : (IpAddr, u16), stream_name : String, numch : u8, sample_rate : VBanSampleRates, format : VBanBitResolution, source_name : String, encoder : u8) -> Option<Self> {
        ...
        let result = VbanSender {
            ...
            socket : match TcpStream::connect(peer){
                Ok(stream) => {
                    trace!("Successfully connected to {}:{}", peer.0, peer.1);
                    stream
                },
                Err(_) => {
                    error!("Could not create to server {}:{} via TCP", peer.0, peer.1);
                    return None
                }
            },
    ...
    /// Handle one iteration of reading from source, composing a VBAN packet and sending via TCP.
    pub fn handle(&mut self){
        ...
        match self.socket.write_all(&vban_packet[..hdr.len()+encoded.len()]){
            Ok(()) => trace!("Successfully sent packet via socket"),
            Err(e) => error!("Error while sending data via socket: {e}")
        }

        self.nu_frame = self.nu_frame.wrapping_add(1);
    }
